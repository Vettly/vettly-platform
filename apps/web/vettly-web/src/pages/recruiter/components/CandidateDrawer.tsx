...
export function CandidateDrawer({
  ...
  const nextStage =
    orderIndex >= 0 && orderIndex < PIPELINE_ORDER.length - 1 ? PIPELINE_ORDER[orderIndex + 1] : undefined;
  ...
  const handleReject = () => {
    if (!window.confirm(`Reject ${displayName}? This moves them to the Rejected stage.`)) return;
    onMove("rejected");
  };
    ...
    <div className="fixed inset-0 z-40 flex justify-end">
      ...
      <div className="relative w-full max-w-[420px] h-full bg-surface-container border-l border-outline-variant flex flex-col">
        <div className="flex items-center justify-between px-5 py-4 border-b border-outline-variant shrink-0">
          <span className="text-[13px] font-bold font-label text-on-surface-variant">Candidate</span>
          <button
            type="button"
            onClick={onClose}
            ...
            <>
              <div className="flex items-center gap-3.5">
                ...
                <div className="min-w-0">
                  <p className="text-[17px] font-semibold font-headline text-on-surface truncate">{displayName}</p>
                  <p className="text-[13px] font-body text-on-surface-variant mt-0.5 truncate">{job?.title ?? "—"}</p>
                </div>
              </div>

              <div className="flex gap-2.5 mt-4">
                <div className="flex-1 bg-surface-container-highest border border-outline-variant rounded-[10px] px-3.5 py-3">
                  <p className="text-[10.5px] font-label uppercase tracking-wide text-on-surface-variant">Match</p>
                ...
                <div className="flex-1 bg-surface-container-highest border border-outline-variant rounded-[10px] px-3.5 py-3">
                  <p className="text-[10.5px] font-label uppercase tracking-wide text-on-surface-variant">Stage</p>
              ...
              <div className="flex flex-col gap-3 mt-4 text-[13px] font-body text-on-surface">
                {profile?.email && (
                  <div className="flex items-center gap-2.5">
                    <span className="material-symbols-outlined text-on-surface-variant" style={{ fontSize: "18px" }}>mail</span>
                    {profile.email}
                  </div>
                )}
                {profile?.location && (
                  <div className="flex items-center gap-2.5">
                    <span className="material-symbols-outlined text-on-surface-variant" style={{ fontSize: "18px" }}>place</span>
                    {profile.location}
                  </div>
                )}
                <div className="flex items-center gap-2.5">
              ...
              {profile && profile.skills.length > 0 && (
                <div className="mt-5">
                  <p className="text-[11px] font-label uppercase tracking-wide text-on-surface-variant mb-2.5">Skills</p>
              ...
              <div className="mt-5 bg-surface-container-highest border border-outline-variant rounded-[10px] p-3.5">
                <p className="text-[11px] font-label uppercase tracking-wide text-on-surface-variant mb-2">Summary</p>
                <p className="text-[13px] leading-relaxed font-body text-on-surface-variant">
                  {profile?.bio || summary?.skillGap || "No summary available."}
                </p>
              </div>

              <div className="mt-5">
                <p className="text-[11px] font-label uppercase tracking-wide text-on-surface-variant mb-2">Notes</p>
                {isEditingNotes ? (
